...
bool fd_init_zlib1(FDLOGMODEL *m) {
    ...
    /// gzip 默认 false
    m->is_ready_gzip = false;
    /// zlib 默认 false
    m->zlib_type = FD_ZLIB_FAIL;
...
//        free(m->strm);
    ...
    m->strm = (z_stream *) malloc(sizeof(z_stream));
    if (m == NULL) {
        printf("m->strm = (z_stream *) malloc(sizeof(z_stream)) failture \n");
        return false;
    }
    memset(m->strm, 0, sizeof(z_stream));

    m->strm->zalloc = Z_NULL;
    m->strm->zfree = Z_NULL;
    m->strm->opaque = Z_NULL;

    // 初始化zlib，具体参数 参考API
    int ret1 = deflateInit2(m->strm, Z_BEST_COMPRESSION, Z_DEFLATED, (15 + 16), 8,
                            Z_DEFAULT_STRATEGY);
    
    if (ret1 == Z_OK) {
        m->is_ready_gzip = true;
        m->zlib_type = FD_ZLIB_INIT;
...
void fd_zlib_end_compress1(FDLOGMODEL *model) {
    // 完成压缩
    fd_zlib1(model, NULL, 0, Z_FINISH);
    // 压缩完成以后,释放空间,但是注意,仅仅是释放deflateInit中申请的空间,自己申请的空间还是需要自己释放
